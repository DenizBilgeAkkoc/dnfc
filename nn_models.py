...
class GeneralModel(nn.Module):
    def __init__(self, encoded_space_dim, target_dim, action_dim, use_image):
        ...
        self.mlp_controller = MLP_3L(encoded_space_dim, 256, 512, 256)
        self.linear = nn.Sequential(
...
class GeneralModelBig(nn.Module):
    def __init__(self, encoded_space_dim, target_dim, action_dim, use_image=False):
        super().__init__()
        # super(GeneralModel, self)._init_()
        ...
        if use_image:
            ...
            self.enc = MLP_4L(target_dim, 480, 480, 480, action_dim)

        # self.mlp_state = MLP(4, 128, encoded_space_dim)
        if use_image: controller_enc_dim = 64
        else: controller_enc_dim = 32
        self.mlp_controller = MLP_4L(encoded_space_dim, 350, 
                                  425, 350, action_dim)
    ...
    def forward(self, target_repr, state):
        ...
        # x_des = self.alexnet(img_tensor)
        x_des = self.enc(target_repr) # (batch_size, encoded_space_dim)
        # zeros=torch.tensor([0 for i in range(8)]).to('cuda')
        # for i in range(32):
        #     
        #     x_des[i] = torch.cat((x_des[i],zeros),0)
        # # x_des = torch.squeeze(x_des, 0)
        diff = x_des - x
        # new = torch.cat((x, diff),1)
...
class MLPBaselineBig(nn.Module):
    def __init__(self, inp_dim, out_dim):
        super().__init__()
